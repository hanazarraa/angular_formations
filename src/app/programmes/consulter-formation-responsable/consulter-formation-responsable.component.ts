...
@Component({
  selector: 'app-consulter-formation-responsable',
  templateUrl: './consulter-formation-responsable.component.html',
  styleUrls: ['./consulter-formation-responsable.component.scss']
})
export class ConsulterFormationResponsableComponent implements OnInit {
...
constructor(private route:ActivatedRoute,private scoreQuizzService:ScoreQuizzService,private router:Router ,private reponseService:ReponseService,private questionService:QuestionService,private changeDetectorRef: ChangeDetectorRef,private http:HttpClient,private consigneService:ConsigneService,private userService:UserService,private _fb:FormBuilder,private toastr: ToastrService,private fb :FormBuilder,private programmeService:ProgrammeService,private CompetenceService:CompetenceService,private NiveauService:NiveauService,private progcompnivService:ProgrammeCompetenceNiveauService
 ...
 this.formateurID=this.route.snapshot.queryParamMap.get('formateur');
...
this.travailService.getTravailByProgrammeIDProprietaire(this.programmeId,this.formateurID)
  .subscribe(data=>{
    this.traveaux=data;
 ...
 /* this.CompetenceService.getCompetenceByProgramme(this.programmeId).subscribe(data=>{
...
.subscribe(data=>{
 ...
 this.questionService.getQuestionByProgrammeIDFormateur(this.programmeId,this.formateurID)
 .subscribe(data=>{
   this.questions=data;
   console.log(this.questions);
...
addReponse() {    
 ...
 if(!this.questions[this.current-1].reponses){
    this.questions[this.current-1].reponses=new  Array<Reponse>();
...
supprimer(){
   this.programmeService.supprimerProgramme(this.programmeId).subscribe(data=>{
       this.closebutton.nativeElement.click();
    this.router.navigateByUrl('responsable/programmes', { state: this.programme.titre});
  });
}
presentation(){
